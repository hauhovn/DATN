...
export function TestScreen({navigation, route}) {
  ...
  const nextQuestion = async () => {
    ...
    if (num == testData.length) await setCurrentQuestion(testData[0]);
    else await setCurrentQuestion(testData[parseInt(currentQuestion.stt)]);
    console.log(currentQuestion);
  };
  const indoQuestion = async () => {
    let num = parseInt(currentQuestion.stt);
    if (num == 1) {
      setCurrentQuestion(testData[testData.length - 1]);
    } else await setCurrentQuestion(testData[num - 2]);
    console.log(currentQuestion);
  };
  //
        ...
        <View style={body.container}>
          <View style={body.title}>
            ...
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate(AppRouter.MENU_QUESTION)}>
              <Icon
