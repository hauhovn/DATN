...
export const Header = ({ GOTO, user, leftHandle, isTeacher = false, name }) => {
        ...
        <View
            ...
            <View style={{ flex: 1, height: 60, justifyContent: 'center' }}>
                <Text style={[mainStyles.title, { fontWeight: 'bold', fontSize: 10 }]}>
                    ...
                    {name ? isTeacher ? 'Giáo viên' : 'Sinh viên' : user[0]?.TenGV !== undefined ? 'Giáo viên' : 'Sinh viên'}
                </Text>
                <Text style={[mainStyles.title, { marginTop: 2, fontSize: 14 }]}>
                    {user[0]?.TenGV} {user[0]?.TenSV} {name}
                </Text>
            </View>
            <TouchableOpacity
                activeOpacity={0.5}
